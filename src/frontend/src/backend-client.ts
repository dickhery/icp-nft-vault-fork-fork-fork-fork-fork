...
const DEFAULT_COLLECTIONS: Array<Collection> = [
  {
    id: 1n,
    name: "DGDG EXT Collection",
    description:
    ...
    symbol: "DGDG",
    canisterId: Principal.fromText("kn6cc-hyaaa-aaaag-qbpna-cai"),
    kind: "External",
    browseInfo: {
      totalSupply: 676n,
      tokenIndexOffset: 0n,
...
function defaultCollections(): Array<Collection> {
  return DEFAULT_COLLECTIONS.map((collection) => ({
    ...collection,
    standard: { ...collection.standard },
    browseInfo:
      collection.browseInfo == null ? undefined : { ...collection.browseInfo },
    dividendConfig:
      collection.dividendConfig == null
        ? undefined
        : { ...collection.dividendConfig },
  }));
}

const COLLECTION_CACHE_KEY = "mintlab.collections.v1";
const USER_NFT_CACHE_PREFIX = "mintlab.user-nfts.v1:";

type CachedCollection = Omit<Collection, "id" | "canisterId" | "browseInfo"> & {
  id: string;
  canisterId: string;
  browseInfo?: {
    totalSupply: string | null;
    tokenIndexOffset: string | null;
  };
};

type CachedWalletNFT = Omit<
  WalletNFT,
  "id" | "collectionId" | "owner" | "registeredAt"
> & {
  id: string;
  collectionId: string;
  owner: string;
...
function mergeCollections(
  ...lists: Array<Array<Collection>>
): Array<Collection> {
  const seen = new Set<string>();
  const merged: Array<Collection> = [];
  for (const list of lists) {
    for (const collection of list) {
      const key = `${collection.id.toString()}:${collection.canisterId.toString()}`;
...
function serializeCollection(collection: Collection): CachedCollection {
  return {
    ...collection,
    id: collection.id.toString(),
    canisterId: collection.canisterId.toString(),
    browseInfo:
      collection.browseInfo == null
        ? undefined
        : {
            totalSupply: collection.browseInfo.totalSupply?.toString() ?? null,
            tokenIndexOffset:
              collection.browseInfo.tokenIndexOffset?.toString() ?? null,
          },
  };
}

function deserializeCollection(value: CachedCollection): Collection {
  return {
    ...value,
    id: BigInt(value.id),
    canisterId: Principal.fromText(value.canisterId),
    browseInfo:
      value.browseInfo == null
        ? undefined
        : {
            totalSupply:
              value.browseInfo.totalSupply == null
                ? null
                : BigInt(value.browseInfo.totalSupply),
            tokenIndexOffset:
              value.browseInfo.tokenIndexOffset == null
                ? null
                : BigInt(value.browseInfo.tokenIndexOffset),
          },
  };
}

function readCachedCollections(): Array<Collection> {
  const storage = localStorageOrNull();
  if (!storage) return [];
  try {
    const raw = storage.getItem(COLLECTION_CACHE_KEY);
    if (!raw) return [];
    return (JSON.parse(raw) as Array<CachedCollection>).map(
...
function writeCachedCollections(collections: Array<Collection>) {
  const storage = localStorageOrNull();
  if (!storage) return;
  storage.setItem(
    COLLECTION_CACHE_KEY,
    JSON.stringify(collections.map(serializeCollection)),
  );
}

function rememberCollections(collections: Array<Collection>) {
  writeCachedCollections(
    mergeCollections(readCachedCollections(), collections),
  );
}

function fallbackCollections(): Array<Collection> {
  return mergeCollections(readCachedCollections(), defaultCollections());
}
...
function userNFTCacheKey(user: Principal): string {
  return `${USER_NFT_CACHE_PREFIX}${user.toString()}`;
}

function serializeWalletNFT(nft: WalletNFT): CachedWalletNFT {
  return {
    ...nft,
    id: nft.id.toString(),
    collectionId: nft.collectionId.toString(),
    owner: nft.owner.toString(),
    registeredAt: nft.registeredAt.toString(),
  };
}

function deserializeWalletNFT(value: CachedWalletNFT): WalletNFT {
  return {
    ...value,
    id: BigInt(value.id),
    collectionId: BigInt(value.collectionId),
    owner: Principal.fromText(value.owner),
    registeredAt: BigInt(value.registeredAt),
  };
}

function readCachedWalletNFTs(user: Principal): Array<WalletNFT> {
  const storage = localStorageOrNull();
  if (!storage) return [];
  try {
    const raw = storage.getItem(userNFTCacheKey(user));
    if (!raw) return [];
    return (JSON.parse(raw) as Array<CachedWalletNFT>).map(
      deserializeWalletNFT,
    );
  } catch {
    return [];
  }
...
function writeCachedWalletNFTs(user: Principal, nfts: Array<WalletNFT>) {
  const storage = localStorageOrNull();
  if (!storage) return;
  storage.setItem(
    userNFTCacheKey(user),
    JSON.stringify(nfts.map(serializeWalletNFT)),
  );
}

function statsFromNFTs(nfts: Array<WalletNFT>): NFTStats {
...
export class Backend implements backendInterface {
  ...
  async addCollection(
    ...
    const collection = fromRawCollection(
      await this.run(() =>
    ...
    rememberCollections([collection]);
    return collection;
  }

  async ensureDefaultCollections(): Promise<Array<Collection>> {
    if (typeof this.actor.ensureDefaultCollections !== "function") {
    ...
    try {
      ...
      if (!isBackendUnavailableError(error)) {
        console.warn("[backend] default collection bootstrap failed", error);
      }
      return fallbackCollections();
    }
  }
  ...
  async createUserCollection(
    ...
    | { __kind__: "err"; err: string }
  > {
    const result = fromCollectionCreationResult(
      await this.run(() =>
    ...
    if (result.__kind__ === "ok") {
      rememberCollections([result.ok.collection]);
    }
    return result;
  }
  ...
  async getNFTStats(user: Principal): Promise<NFTStats> {
    try {
      return await this.run(() => this.actor.getNFTStats(user));
    } catch (error) {
      if (!isBackendUnavailableError(error)) throw error;
      return statsFromNFTs(readCachedWalletNFTs(user));
    }
  }
  ...
  async getUserNFTs(user: Principal): Promise<Array<WalletNFT>> {
    try {
      ...
      const nfts = result.map(fromRawWalletNFT);
      writeCachedWalletNFTs(user, nfts);
      return nfts;
    } catch (error) {
      if (!isBackendUnavailableError(error)) throw error;
      return readCachedWalletNFTs(user);
    }
  }
  ...
  async listCollections(): Promise<Array<Collection>> {
    try {
      ...
      console.warn("[backend] listCollections failed", error);
      return fallbackCollections();
    }
